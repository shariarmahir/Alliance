 ...
 *
 * This module is the single source of the document's design — the letter
 * format, the bordered item table with its Picture column, the terms block and
 * the signature footer. The email attachment is produced from the very same
 * builder via quotationPdfToBase64, so the copy an admin saves and the copy a
 * customer receives are the same document by construction.
 */
